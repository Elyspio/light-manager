...
import {socketIoServer} from "./LightServer";
import {socketEvents} from "../../config/socket";
import {Ip} from "../../module/light/types";

export namespace Responses {
    export const notSupported = (res: Response) =>
        res.status(500).send({
            error: "NIY",
            message: "Not implemented yet",
        });

    export const unknownLight = (res: Response, ip: Ip) =>
        res.status(500).send({
            error: "NoLight",
    ...
    export const ok = (res: Response, light: Light | Light[]) => {
        if ((light as Light).ip && (light as Light).name) {
            ...
            const c = socketIoServer.sockets.emit(
                socketEvents.updateAll,
                (light as Light[]).map((l) => l.json())
            );
            res.end();
        }
    };
